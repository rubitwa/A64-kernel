...
#define LED_POWER_CONTROL

#ifdef LED_POWER_CONTROL
struct led_power {
	unsigned int vol;
	const char *name;
};
...
static int gpio_led_probe(struct platform_device *pdev)
{
	...
	u32 led_power_num;
	const char *led_power_name;
	char led_power_index[20];
	char led_power_vol_index[20];
	struct regulator *led_power;
...
#ifdef LED_POWER_CONTROL
	ret = of_property_read_u32(np, "led_power_num", &led_power_num);
	if (ret < 0) {
	...
	if(led_power_num > 0) {
		for(i = 0; i < led_power_num; i++){
			snprintf(led_power_index, 15, "led%u_power", (i+1));
			if (of_property_read_string(np, led_power_index, &led_power_name)){
				dev_err(dev, "%s: %s not used\n", __func__, led_power_index);
				continue;
			}
			else {
				pr_info("%s, %s power name is %s\n", __func__, led_power_index, led_power_name);

				snprintf(led_power_vol_index, 15, "led%u_power_vol", (i+1));
				if(of_property_read_u32(np, led_power_vol_index, &led_power_vol)) {
					dev_err(dev, "%s: %s not used\n", __func__, led_power_vol_index);
					continue;
				}

				pr_info("%s, %s power vol is %d\n", __func__, led_power_vol_index, led_power_vol);

				led_power = regulator_get(dev, led_power_name);
				if (!IS_ERR(led_power)) {
					ret = regulator_set_voltage(led_power, (int)(led_power_vol)*1000, (int)(led_power_vol)*1000);
					if(ret) {
						dev_err(dev, "regulator %s set voltage failed\n", led_power_name);
						regulator_put(led_power);
						return ret;
					}
					
					ret = regulator_enable(led_power);
					if (ret < 0){
